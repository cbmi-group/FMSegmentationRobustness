...
class PGD_attack():
    ...
    def perturb(self, original_images, labels, scale, device, reduction4loss='mean', random_start=False):
        # original_images: values are within self.min_val and self.max_val

        # The adversaries created from random close points to the original data
        if self.model.output_c == 1:
            criterion = nn.BCEWithLogitsLoss()
        else:
            criterion = nn.CrossEntropyLoss()
        criterion = criterion.to(device=device)
        # gray_imgs = original_images[:, 0, :, :].view(original_images.shape[0], -1)
        # std = torch.mean(torch.std(gray_imgs, 1))
        if random_start:
            rand_perturb = torch.FloatTensor(original_images.shape).uniform_(
                -self.epsilon, self.epsilon)
            # rand_perturb = tensor2cuda(rand_perturb)
            rand_perturb = rand_perturb.to(device=device, dtype=torch.float32)
        ...
        with torch.enable_grad():
            for _iter in range(self.max_iters):
                ...
                if reduction4loss == 'none':
                    ...
                    grad_outputs = torch.ones(loss.shape).to(device=device, dtype=torch.float32)
                ...
                x.data += self.alpha/scale * torch.sign(grads.data)

                # the adversaries' pixel value should within max_x and min_x due 
                # to the l_infinity / l2 restriction
                x = project(x, original_images, self.epsilon/scale, self._type)
