...
class ICNet(BaseModel):
	...
	def __init__(self, output_c = 2 ,backbone='resnet18', pretrained_backbone=None):
		...
		if 'resnet' in backbone:
			...
			# self.zlq = ConvBlock(in_channels=3, out_channels=32, kernel_size=3, stride=2, padding=1, bias=False)
			...
			# Cascade Feature Fusion
			self.cff_24 = CascadeFeatFusion(low_channels=stage5_channels//4, high_channels=128, out_channels=128, num_classes=output_c)
			self.cff_12 = CascadeFeatFusion(low_channels=128, high_channels=64, out_channels=128, num_classes=output_c)

			# Classification
			self.conv_cls = nn.Conv2d(in_channels=128, out_channels=output_c, kernel_size=1, bias=False)
	...
	def forward(self, input):
		# Sub1'
		# input = F.interpolate(input, size=[258,258], mode='bilinear', align_corners=True)
		# print(input.shape)
		# x = self.zlq(input)
		# print(x.shape)
		x_sub1 = self.conv_sub1(input)   # 1/8 (64,32,32), (256-3+2)/2 = 257/2 = 128.5, 原则向下取整，故最终输出为 32*32
		# print('+++++++x_sub1',x_sub1.size())

		# Sub2
		x_sub2 = F.interpolate(input, scale_factor=0.5, mode='bilinear', align_corners=True)
		_, x_sub2, _, _ = self.base_forward(x_sub2)  # 1/16 (128,16,16)
		# print('+++++++x_sub2', x_sub2.size())

		# Sub4
		x_sub4 = F.interpolate(input, scale_factor=0.25, mode='bilinear', align_corners=True)
		_, _, _, x_sub4 = self.base_forward(x_sub4)  # 1/32   (512,8,8)
		# print('+++++++x_sub4', x_sub4.size())
		# add PyramidPoolingModule
		x_sub4 = self.ppm(x_sub4)
		x_sub4 = self.conv_sub4_reduce(x_sub4)  # 1/32   (128,8,8)

		# Output
		# Cascade Feature Fusion
		x_cff_24, x_24_cls = self.cff_24(x_sub4, x_sub2)  # 1/16
		x_cff_12, x_12_cls = self.cff_12(x_cff_24, x_sub1)   # 1/8

		# Classification
		x_cff_12 = F.interpolate(x_cff_12, scale_factor=2, mode='bilinear',
								 align_corners=True)
		x_124_cls = self.conv_cls(x_cff_12)  # 1/4 (1,64,64)
		# print('+++++++x_124_cls', x_124_cls.size())

		out = F.interpolate(x_124_cls, scale_factor=4, mode='bilinear', align_corners=True)

		if self.training:
			return x_124_cls, x_12_cls, x_24_cls  # 1/4 - 1/8 - 1/16
		else:
	...
	def base_forward(self, x):
		...
		c2 = self.backbone.layer2(c1)  # (128, 16,16)
		c3 = self.backbone.layer3(c2)  #
		c4 = self.backbone.layer4(c3)  # (512, 8, 8)
